...
function tR_calc(A, B, C, L, λ, β, prog, opt, gas)
	...
	rM(x,t,L,λ) = 64 * sqrt(prog.Fpin_itp(t)^2 - x/L*(prog.Fpin_itp(t)^2-prog.pout_itp(t)^2)) / (prog.Fpin_itp(t)^2-prog.pout_itp(t)^2) * λ^2/L * GasChromatographySimulator.viscosity(x, t, T_itp, gas, vis=opt.vis) * prog.T_itp(x, t)
