#!/usr/bin/env julia
"""
Benchmark script for multistart optimization with method_m4 using single chromatograms.
...
Usage:
    julia benchmark_multistart_m4.jl [data_file] [multistart_n] [maxiters] [maxtime]

Examples:
    julia benchmark_multistart_m4.jl
    julia benchmark_multistart_m4.jl data/meas_df05_Rxi5SilMS.csv 10
    julia benchmark_multistart_m4.jl data/meas_df05_Rxi5SilMS.csv 10 5000 300.0
"""
...
println("=" ^ 80)
println("Benchmark: method_m4 with multistart (single chromatograms)")
println("=" ^ 80)
println("\nConfiguration:")
...
for meas_idx in 1:n_measurements
    ...
    # Benchmark without multistart - simplified method_m4 logic
    println("  Running without multistart...")
    time_no_multistart = @elapsed begin
        # Initialize d with quick dKcentric_single pass
        ...
        θchar_current = res_dKcentric_init.θchar
        ΔCp_current = res_dKcentric_init.ΔCp
        
        # Alternating optimization (simplified - just one iteration for benchmark)
        new_col = GasChromatographySimulator.Column(meas_single[1].L, d_current, meas_single[1].df, meas_single[1].sp, meas_single[1].gas)
        res_no_multistart = RetentionParameterEstimator.estimate_parameters(
            meas_single[3], meas_single[4], new_col, meas_single[2], 
            Tchar_current, θchar_current, ΔCp_current;
...
# Format loss values appropriately (for summary)
format_loss(loss) = if loss < 1e-6
    @sprintf("%.2e", loss)
elseif loss < 1.0
    @sprintf("%.8f", loss)
