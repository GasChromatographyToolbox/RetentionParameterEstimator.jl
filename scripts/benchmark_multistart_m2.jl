#!/usr/bin/env julia
"""
Benchmark script for multistart optimization with method_m2 using single chromatograms.
...
Usage:
    julia benchmark_multistart_m2.jl [data_file] [multistart_n] [maxiters] [maxtime]

Examples:
    julia benchmark_multistart_m2.jl
    julia benchmark_multistart_m2.jl data/meas_df05_Rxi5SilMS.csv 10
    julia benchmark_multistart_m2.jl data/meas_df05_Rxi5SilMS.csv 10 5000 300.0
"""
...
println("=" ^ 80)
println("Benchmark: method_m2 with multistart (single chromatograms)")
println("=" ^ 80)
println("\nConfiguration:")
...
for meas_idx in 1:n_measurements
    ...
    Tchar_est, θchar_est, ΔCp_est, Telu_max = RetentionParameterEstimator.estimate_start_parameter(
        meas_single[3], meas_single[1], meas_single[2]; time_unit=meas_single[6]
    )
    
    # Benchmark without multistart
    println("  Running without multistart...")
    time_no_multistart = @elapsed begin
        res_dKcentric_no_multistart = RetentionParameterEstimator.estimate_parameters(
        ...
        d_mean = mean(res_dKcentric_no_multistart.d)
        new_col = GasChromatographySimulator.Column(meas_single[1].L, d_mean, meas_single[1].df, meas_single[1].sp, meas_single[1].gas)
        
        # Second pass: optimize K-centric parameters with fixed d
        res_no_multistart = RetentionParameterEstimator.estimate_parameters(
            meas_single[3], meas_single[4], new_col, meas_single[2], 
            res_dKcentric_no_multistart.Tchar, res_dKcentric_no_multistart.θchar, res_dKcentric_no_multistart.ΔCp;
    ...
    time_multistart = @elapsed begin
        ...
        # Calculate mean d and create new column (mimicking method_m2)
        d_mean = mean(res_dKcentric_multistart.d)
        new_col = GasChromatographySimulator.Column(meas_single[1].L, d_mean, meas_single[1].df, meas_single[1].sp, meas_single[1].gas)
        
        # Second pass: optimize K-centric parameters with fixed d
        res_multistart = RetentionParameterEstimator.estimate_parameters(
            meas_single[3], meas_single[4], new_col, meas_single[2], 
            res_dKcentric_multistart.Tchar, res_dKcentric_multistart.θchar, res_dKcentric_multistart.ΔCp;
            mode="Kcentric_single", pout=meas_single[5], time_unit=meas_single[6],
            method=RetentionParameterEstimator.NewtonTrustRegion(), 
            opt=RetentionParameterEstimator.std_opt,
            maxiters=maxiters, maxtime=maxtime, multistart_n=multistart_n
